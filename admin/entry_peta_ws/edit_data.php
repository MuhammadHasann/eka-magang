<?php
$id = $_GET['id'];
$select = mysqli_query($koneksi, "SELECT * FROM entry_peta_ws WHERE id_entry_peta_ws = '$id'");
...
if (isset($_POST['save'])) {
  ...
  $query = mysqli_query($koneksi, "UPDATE entry_peta_ws 
  SET id_entry_peta_ws = '$id_entry_peta_ws',  = kabupaten = '$kabupaten', kecamatan = '$kecamatan', desa = '$desa', nama_sls = '$nama_sls', scan '$scan',
  WHERE id_entry_peta_ws = '$id'");
  if ($query) {
?>
    <script>
      window.location = "entry_peta_ws.php?msg=success";
    </script>
  <?php
  } else {
